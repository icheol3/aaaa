...
class StudyHubApp {
    ...
    navigateToPage(page) {
        ...
        switch (page) {
            ...
            case 'planner':
                ...
                break;
            case 'statistics':
                this.updateCharts(); // 통계 페이지 진입 시 차트 데이터만 업데이트
                break;
            case 'community':
    ...
    loadQuestions() {
        ...
        qnaList.innerHTML = this.data.questions.map(question => `
            <div class="question-card bg-white dark:bg-gray-800 rounded-lg shadow p-6" data-question-id="${question.id}">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">${this.data.user?.name ? question.author : '익명'} • ${this.formatDate(question.createdAt)}</p>
                        </div>
                    </div>
                ...
                <div class="border-t dark:border-gray-600 pt-3">
                    ...
                    <div class="space-y-2">
                        ${question.answers.map(answer => `
                            <div class="answer-card bg-gray-100 dark:bg-gray-700 p-3 rounded-md">
                                <div class="flex items-center space-x-2 mb-1">
                                    <div class="w-6 h-6 bg-green-500 rounded-full flex items-center justify-center">
                                        <span class="text-white text-xs font-bold">${answer.author.charAt(0).toUpperCase()}</span>
                                    </div>
                                    <span class="font-medium text-sm text-gray-800 dark:text-gray-200">${answer.author}</span>
                                    <span class="text-xs text-gray-500">${this.formatDate(answer.createdAt)}</span>
                                </div>
                                <p class="text-sm ml-8 text-gray-700 dark:text-gray-300">${answer.content}</p>
                            </div>
                        `).join('')}
                    </div>
                </div>
    ...
    loadTasks() {
        ...
        taskList.innerHTML = this.data.tasks.map(task => `
            <div class="task-item flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded-md ${task.completed ? 'opacity-60 line-through' : ''}" data-task-id="${task.id}">
                <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 rounded" ${task.completed ? 'checked' : ''} 
                       onchange="studyHub.toggleTask('${task.id}')">
                <span class="flex-1 ml-2 text-gray-800 dark:text-gray-200">${task.content}</span>
    ...
    recordStudyTime() {
        ...
        this.loadStudyRecords();
        this.updateDashboard();
        this.updateCharts(); // 학습 기록 시 차트 업데이트
    ...
    loadStudyRecords() {
        ...
        recordsContainer.innerHTML = todayRecords.map(record => `
            <div class="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700 rounded">
                <span class="font-medium text-gray-800 dark:text-gray-200">${record.subject}</span>
    ...
    // 대시보드 업데이트
    updateDashboard() {
        // 사용자 정보가 없으면 대시보드 업데이트 안함
        if (!this.data.user) {
            document.getElementById('todayStudyTime').textContent = '0분';
            document.getElementById('completedTasks').textContent = '0';
            document.getElementById('myQuestions').textContent = '0';
            document.getElementById('recentQnA').innerHTML = '<div class="text-gray-500 dark:text-gray-400 text-center py-8">로그인하여 활동을 시작해보세요!</div>';
            document.getElementById('todayTasks').innerHTML = '<div class="text-gray-500 dark:text-gray-400 text-center py-8">로그인하여 활동을 시작해보세요!</div>';
            return;
        }
        ...
        if (recentQuestions.length === 0) {
            ...
            recentQnA.innerHTML = recentQuestions.map(q => `
                <div class="border-b dark:border-gray-600 pb-2 mb-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 rounded p-2 transition-colors" onclick="studyHub.navigateToQuestion('${q.id}')">
                    <h4 class="font-medium text-sm text-gray-800 dark:text-gray-200">${q.title}</h4>
        ...
        if (incompleteTasks.length === 0) {
            ...
            todayTasks.innerHTML = incompleteTasks.map(task => `
                <div class="flex items-center space-x-2">
                    <input type="checkbox" class="w-4 h-4 text-blue-600 rounded" onchange="studyHub.toggleTask('${task.id}')">
                    <span class="text-sm text-gray-800 dark:text-gray-200">${task.content}</span>
                </div>
            `).join('');
        }
    }
    ...
    initWeeklyStudyChart() {
        ...
        const last7Days = [];
        const studyData = [];
        const textColor = document.documentElement.classList.contains('dark') ? '#e2e8f0' : '#333';
        ...
        if (this.charts.weeklyStudy) {
            ...
            this.charts.weeklyStudy.data.labels = last7Days;
            this.charts.weeklyStudy.data.datasets[0].data = studyData;
            // 테마 변경에 따른 텍스트/그리드 색상 업데이트
            this.charts.weeklyStudy.options.scales.y.ticks.color = textColor;
            this.charts.weeklyStudy.options.scales.y.grid.color = gridColor;
            this.charts.weeklyStudy.options.scales.x.ticks.color = textColor;
            this.charts.weeklyStudy.options.scales.x.grid.color = gridColor;
            this.charts.weeklyStudy.options.scales.y.title.color = textColor;
            this.charts.weeklyStudy.options.plugins.legend.labels.color = textColor; // 레전드 텍스트 색상 추가

            this.charts.weeklyStudy.update();
        } else {
            // 차트가 없으면 새로 생성
                    ...
                    scales: {
                        y: {
                            ...
                            title: {
                                display: true,
                                text: '시간',
                                color: textColor // Y축 제목 색상
                            },
                            ticks: {
                                color: textColor // Y축 눈금 텍스트 색상
                            },
                            grid: {
    ...
    initSubjectChart() {
        ...
        if (this.charts.subject) {
            // 기존 차트가 있으면 데이터만 업데이트
            this.charts.subject.data.labels = labels;
            this.charts.subject.data.datasets[0].data = data;
            this.charts.subject.data.datasets[0].backgroundColor = colors.slice(0, labels.length);
            // 테마 변경에 따른 레전드 텍스트 색상 업데이트
            this.charts.subject.options.plugins.legend.labels.color = textColor;
    ...
    // 수면 패턴 차트 (시뮬레이션 데이터) 데이터 업데이트 및 렌더링
    initSleepChart() {
        ...
        if (this.charts.sleep) {
            // 기존 차트가 있으면 데이터만 업데이트
            this.charts.sleep.data.labels = last7Days;
            this.charts.sleep.data.datasets[0].data = sleepData;
            // 테마 변경에 따른 텍스트/그리드 색상 업데이트
            this.charts.sleep.options.scales.y.ticks.color = textColor;
            this.charts.sleep.options.scales.y.grid.color = gridColor;
            this.charts.sleep.options.scales.x.ticks.color = textColor;
            this.charts.sleep.options.scales.x.grid.color = gridColor;
            this.charts.sleep.options.scales.y.title.color = textColor;
            this.charts.sleep.options.plugins.legend.labels.color = textColor; // 레전드 텍스트 색상 추가
    ...
    // 차트 업데이트 (데이터만 갱신)
    updateCharts() {
        // 각 차트의 init 함수를 호출하여 데이터와 옵션을 업데이트하고 차트를 갱신
        this.initWeeklyStudyChart();
        this.initSubjectChart();
        this.initSleepChart();
    }
        ...
        postsContainer.innerHTML = this.data.communityPosts.map(post => `
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                <div class="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                        <h4 class="font-semibold text-gray-800 dark:text-gray-200">${post.author}</h4>
